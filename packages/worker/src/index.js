...
import {
  concatenateChunks, base64ToBytes, jsonResponse,
} from '../../../shared/utils.js';
import {
  getInterviewContext, saveInterviewTurn, cloudflareCapabilities,
...
async function handleTranscription(request, env) {
  ...
  const result = await transcribe(env, bytes, {
    model: model || 'auto', language: language || 'en', prompt: prompt || undefined, temperature: temperature ?? 0,
  }, mime || 'audio/webm');
    ...
    try {
      ...
      if (data.type === 'start') {
        ...
        sessions.set(webSocket, {
          chunks: [], startTime: Date.now(), lastAt: Date.now(),
          model: data.model || 'auto', language: data.language || 'en', sessionId,
        });
      ...
      if (data.type === 'end' && sessions.has(webSocket)) {
        ...
        webSocket.send(JSON.stringify({
          text: result.text, model: result.model, segments: result.segments || [],
          session_id: session.sessionId, latency_ms: Date.now() - session.startTime,
        }));
        return;
      }
      ...
      try { webSocket.send(JSON.stringify({ error: 'Internal server error' })); } catch {}
    }
  });
...
async function handleSessionRoute(request, env, ctx, path) {
  ...
  if (path === '/api/session' && request.method === 'POST') {
    ...
    return jsonResponse({
      ...
      status: 'active',
      persisted: Boolean(env.INTERVIEW_DB),
    }, 201, request);
  }
  ...
  if (request.method === 'GET' && action === 'context') {
    ...
    const turns = await getInterviewContext(env, id, limit);
    return jsonResponse({ session_id: id, turns }, 200, request);
  }
  ...
  if (request.method === 'GET' && action === 'report') {
    ...
    if (!row) return jsonResponse({ session_id: id, status: 'pending', report: null }, 200, request);
    return jsonResponse({ session_id: id, status: 'completed', report: JSON.parse(row.report_json), created_at: row.created_at, updated_at: row.updated_at }, 200, request);
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      if (path === '/dashboard') {
        return new Response(DASHBOARD_HTML, { headers: { 'Content-Type': 'text/html;charset=utf-8', ...corsHeaders(request) } });
      }
      ...
      if (path === '/' || path === '/health') {
        ...
        return jsonResponse({
          ...
          engine: 'Cloudflare Workers AI (+ optional AI Gateway fallback)', models_count: modelCount,
          dashboard: url.origin + '/dashboard', capabilities: cloudflareCapabilities(env),
          auth: 'API key optional for direct-use mode',
          endpoints: {
            'POST /v1/chat/completions': 'OpenAI-compatible chat',
            'POST /api/answer': 'Realtime interview answer',
            'POST /api/session': 'Create interview session',
            'GET /api/session/:id': 'Get session/report',
            'GET /api/session/:id/context': 'Get persisted turns',
            'POST /api/session/:id/finalize': 'Start post-interview analysis',
            'GET /api/session/:id/report': 'Get interview evaluation report',
            'POST /v1/audio/transcriptions': 'Speech-to-text',
            'POST /v1/audio/stream': 'Streaming STT',
          },
        });
      }

      const sessionResponse = await handleSessionRoute(request, env, ctx, path);
      if (sessionResponse) return sessionResponse;

      if (path === '/v1/models' && request.method === 'GET') {
        return jsonResponse({ object: 'list', data: Object.entries(MODELS).map(([key, m]) => ({
          id: key, model_id: m.id, name: m.name, params: m.params || null, context: m.context,
          max_tokens: m.max_tokens || null, speed: m.speed, caps: m.caps, bestFor: m.bestFor,
        })) });
      }

      if (path === '/v1/tasks' && request.method === 'GET') {
        return jsonResponse({ tasks: Object.entries(ROUTING_TABLE).map(([key, r]) => ({
          id: key, label: r.label, primary_model: r.chains[0]?.model,
          models: r.chains.map((c) => ({ model: c.model, model_id: MODELS[c.model]?.id, reason: c.reason })),
        })) });
      }
      ...
      if (path === '/v1/keys' && request.method === 'POST') {
        ...
        const keyData = await createApiKey(env, {
          name: body.name, expires_at: body.expires_at || null, rate_limit: body.rate_limit ?? 0,
      ...
      if (path === '/v1/keys' && request.method === 'GET') {
        if (!request._keyData || request._keyData.id === 'anon') return jsonResponse({ error: 'Authentication required to list keys' }, 401, request);
        return jsonResponse({ keys: await listApiKeys(env) });
      }

      if (path === '/v1/keys/revoke' && request.method === 'POST') {
        if (!request._keyData || request._keyData.id === 'anon') return jsonResponse({ error: 'Authentication required to revoke keys' }, 401, request);
        const body = await readBody(request);
        if (!body) return jsonResponse({ error: 'Invalid JSON body' }, 400, request);
      ...
      if (path === '/v1/keys' && request.method === 'DELETE') {
        if (!request._keyData || request._keyData.id === 'anon') return jsonResponse({ error: 'Authentication required to delete keys' }, 401, request);
        const body = await readBody(request);
        if (!body) return jsonResponse({ error: 'Invalid JSON body' }, 400, request);
      ...
      if (path === '/ask' && request.method === 'POST') {
        ...
        const result = await routeRequest({ messages: [{ role: 'user', content: question }], max_tokens: body.max_tokens || 0, temperature: body.temperature ?? 0.7, stream: false, task_type: body.task_type }, env);
      ...
      if (path === '/api/answer' && request.method === 'POST') {
        ...
        if (Array.isArray(body.history)) messages.push(...body.history.slice(-12).filter((m) => m && m.role && m.content));
        messages.push({ role: 'user', content: question });
        return respondToChat({
          model: body.model || 'auto', messages, max_tokens: body.max_tokens || 512,
          temperature: body.temperature ?? 0.3, stream: body.stream !== false, task_type: 'interview',
          sessionId: body.sessionId || null, turnId: body.turnId || null,
          resume: body.resume, jobDesc: body.jobDesc, targetName: body.targetName, participants: body.participants,
          candidateName: body.candidateName, roleTitle: body.roleTitle, company: body.company,
        }, env, ctx, request, {
          sessionId: body.sessionId || null, turnId: body.turnId || null, question,
          candidateName: body.candidateName, roleTitle: body.roleTitle, company: body.company,
        });
      }
  ...
  websocket: {
    async handle(webSocket, env) { await handleVoiceSocket(webSocket, env); }
  }
};
...
function buildChatResponse(response, metadata, request) {
  const base = {
    id: `chatcmpl-${crypto.randomUUID()}`, object: 'chat.completion', model: metadata.model_used,
    task_type: metadata.task_type, task_label: metadata.task_label,
    routing: { reason: metadata.routing_reason, attempts: metadata.all_attempts },
    ...
    try {
      for await (const chunk of response.stream) {
        ...
        if (text) {
          fullAnswer += text;
          const payload = {
            id: completionId, object: 'chat.completion.chunk', model: metadata.model_used,
            task_type: metadata.task_type,
            choices: [{ index: 0, delta: { content: text }, finish_reason: null }],
          };
          if (meta?.sessionId) payload.session_id = meta.sessionId;
          if (meta?.turnId) payload.turn_id = meta.turnId;
          await writer.write(encoder.encode(`data: ${JSON.stringify(payload)}\n\n`));
        }
      }
      if (meta?.sessionId && fullAnswer && envForMeta(meta)) {
        // Persistence is intentionally performed after the final token so it never delays the first token.
        const persistPromise = saveInterviewTurn(envForMeta(meta), {
          sessionId: meta.sessionId, turnId: meta.turnId, question: meta.question || '', answer: fullAnswer,
          taskType: metadata.task_type, model: metadata.model_used, latencyMs: metadata.latency_ms,
          candidateName: meta.candidateName, roleTitle: meta.roleTitle, company: meta.company,
        });
        if (ctx) ctx.waitUntil(persistPromise);
      ...
      if (meta?.sessionId) finish.session_id = meta.sessionId;
      if (meta?.turnId) finish.turn_id = meta.turnId;
...
// Kept separate from the request handler so the stream pump can safely access env
// without changing the public helper signature used elsewhere.
function envForMeta(meta) { return meta?.env || null; }
