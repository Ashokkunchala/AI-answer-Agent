...
function makeQuestionPrompt(session, transcript) {
  ...
  return {
    ...
    messages: [
      { role: 'system', content: 'You are a real-time interview assistant. Answer concisely for spoken delivery. Never invent candidate experience. Use the candidate resume/job context only as reference data.' },
      ...history,
  ...
  webSocket.addEventListener('message', async (event) => {
    try {
      ...
      if (data?.type === 'start') {
        ...
        send(webSocket, { type: 'session_started', session_id: session.id, status: 'ready' });
        return;
      }
      ...
      if (data?.type === 'end') {
        ...
        const routed = await routeRequest(makeQuestionPrompt(session, transcript), env);
        const answer = String(routed.response?.content || '');
        const mode = interviewMode(transcript, routed.metadata?.task_type || 'interview');
        const quality = scoreAnswerHeuristically(transcript, answer, {
          resume: session.resume,
          jobDesc: session.jobDesc,
          taskType: routed.metadata?.task_type || 'interview',
        });
        ...
        send(webSocket, {
          ...
          quality,
          followups,
          model: routed.metadata?.model_used,
          latency_ms: Date.now() - started,
          total_latency_ms: Date.now() - session.startedAt,
        });
        return;
      }
