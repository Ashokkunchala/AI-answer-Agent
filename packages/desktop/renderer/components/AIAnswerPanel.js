...
export class AIAnswerPanel extends Component {
  ...
  updateLastAnswer(text) {
    ...
    if (last) {
      ...
      if (answerEl) {
        answerEl.innerHTML = this._formatAnswer(text);
  ...
  _scrollToBottom() {
    // Scroll the answer panel container to show latest content
    const container = this.el?.closest('.ai-tool-answer');
    if (container) {
  ...
  _applyStyles() {
    ...
    s.id = 'aap-styles';
    s.textContent = `
      .ai-answer-panel { display:flex; flex-direction:column; gap:8px; padding:8px 12px; height:100%; }
      .aap-empty { text-align:center; padding:30px 20px; color:var(--text2,#64748b); }
      .aap-empty-icon { font-size:2em; margin-bottom:8px; }
      .aap-answers { display:flex; flex-direction:column; gap:8px; overflow-y:auto; flex:1 1 auto; min-height:0; }
      ...
      .aap-card-question { font-size:.82em; color:var(--accent2,#a78bfa); font-weight:600; word-break:break-word; }
      .aap-card-meta { font-size:.7em; color:var(--text2,#64748b); flex-shrink:0; }
      .aap-card-answer { font-size:.85em; color:var(--text,#e2e8f0); line-height:1.6; word-break:break-word; max-height:400px; overflow-y:auto; }
      ...
      .aap-answers::-webkit-scrollbar { width:6px; }
      .aap-answers::-webkit-scrollbar-track { background:transparent; }
      .aap-answers::-webkit-scrollbar-thumb { background:rgba(255,255,255,0.1); border-radius:3px; }
      .aap-answers::-webkit-scrollbar-thumb:hover { background:rgba(255,255,255,0.2); }
    `;
